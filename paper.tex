...
The basic idea of this approach is that multiple parties split their secrets into so called shares and then distribute them to the other parties. Computations are executed on these shares, and after the end it is possible to reconstruct the result from the different shares without revealing information about the initial secrets. There is a huge family of protocols build on this approach, and it is still evolving every day. This paper will focus on the newest of these protocols that were developed in the last 10 years, especially on the SPDZ like protocols.


TODO: Finite field Fp
TODO: Lagrange interpolation? T+1 shares needed to reconstruct secret; choose random polynomial out of finite field 
