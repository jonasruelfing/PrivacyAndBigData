...
\item Now each party \( P_j\) holds a share of \( a\) as tuple \(\langle a_i\rangle = (\delta_a,a_i,\gamma(a)_i )\)
\end{itemize}
...
Correctness: Assume that $n-1$ secret shares are known to an adversary, so all secret shares except one $a_x \in \mathcal{F}_p $. Then the adversary can calculate SUM=A-A_x. Nevertheless, because $a_x$ was drawn from an independent distribution, knowing SUM reveals not a single bit of information about $a$.\\


In SCALE-MAMBA, this value is then represented as followin tuple:\\
...
\end{itemize}
 ...
 and the corresponding tag \( \gamma(a)\). Then th
Sharing of values:\\
...
Following, it is going to be demonstrated, how addition and multiplication are done in this environment. This is sufficient, because a group with addition and multiplication is already touring complete, so there is no need for any other operation.  Furthermore, addition and multiplications with constants are also discussed.\\
